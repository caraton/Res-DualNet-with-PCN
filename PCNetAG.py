...
class PCBlockAG(nn.Module):
    ...
    def __init__(
        ...
        if stride != 1 or in_channels != PCBlockAG.expansion * out_channels:
    ...
    def infer(self, x, decay, infer_rate):
        ...
        dX = self.DP2.backward(self.Xs["PW"], self.pred_errors["DP2"])
        ...
        # @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        dX = self.DP1.backward(self.Xs["input"], self.pred_errors["DP1"])
    ...
    def update_weights(self):
        return [
            self.DP1.update_weights(self.Xs["input"], self.Xs["DP1"]),
            self.PW.update_weights(self.pred_errors["PW"]),
            self.DP2.update_weights(self.Xs["PW"], self.Xs["DP2"]),
        ]
